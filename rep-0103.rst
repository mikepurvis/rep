...
Geography
---------
Systems representing a cartesian geographic location should do so in UTM,
according to the `east north up`_ convention:

* X east
* Y north
* Z up

Orientations should similarly be expressed according to the right hand
rule. Specifically, yaw should be zero when frame's forward x-axis points
each, and the yaw should increase positively as that frame rotates toward
pointing north.
